		...
		init : function( options )
		{
			...
			var menuItems = { 'fonteffects': true,
						   ...
						   'insert_link': { "modal": true,
						   					"modalId": "InsertLink", 
											"icon":"fa fa-link", 
											"tooltip": "Inserir endereço", 
											"modalHeader": "Criar link",
											...
											"onSave":function(){
												...
												if(targetURL ==''){
													methods.showMessage.apply(editorObj,["errMsg","Insira um endereço"]);
													return false;
												}												
												if(!targetURL.match(urlPattern)){
													methods.showMessage.apply(editorObj,["errMsg","Insira um endereço válido"]);
													return false;
												}													
												if(range=='' && targetText==''){ 
